...
@section('content')
    <div class="max-w-2xl">
        ...
        <div class="bg-white/70 backdrop-blur-xl rounded-2xl border border-gray-200/50 shadow-sm overflow-hidden">
            <div class="px-8 py-6 border-b border-gray-100 bg-gradient-to-r from-amber-50/50 to-orange-50/50">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="text-lg font-bold text-gray-900">Edit: {{ $product->nama_produk }}</h3>
                        <p class="text-xs text-gray-500">Perbarui data produk sesuai kebutuhan</p>
                    </div>
                </div>
            </div>

            <form method="POST" action="{{ route('products.update', $product) }}" class="p-8 space-y-6" id="edit-product-form">
                @csrf
                @method('PUT')
                ...
                <div class="flex items-center gap-3 pt-4 border-t border-gray-100">
                    <button type="submit" id="btn-update-produk"
                        class="px-8 py-3.5 rounded-2xl bg-gradient-to-r from-amber-400 to-orange-500 text-white text-sm font-semibold
